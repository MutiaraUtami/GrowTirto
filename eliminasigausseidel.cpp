...
#define EPSILON 0 // toleransi kesalahan
...
int main() {
    ...
    do {
        ...
        if (fabs(x_new[0] - x[0]) < EPSILON && fabs(x_new[1] - x[1]) < EPSILON && fabs(x_new[2] - x[2]) < EPSILON) {
            break;
        }
    ...
    if (iter == MAX_ITER) {
        ...
        cout << "Hasil akhirnya: x1 = " << x_new[0] << ", x2 = " << x_new[1] << ", x3 = " << x_new[2] << endl;
    }
